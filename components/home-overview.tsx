...
const quickAnswers: Record<string, string> = {
  "Who is Vedanand?":
    "Vedanand is a Backend Engineer and Full Stack Developer from Noida, focused on distributed systems, microservices, and scalable architectures.",
  "How much experience does he have?":
    "Vedanand is currently a fresher with strong project based experience. He has built multiple production-style systems, including WorkWizard, AegisID, and a real-time collaborative drawing app.",
  "Which languages does he use?":
    "Vedanand works with JavaScript, TypeScript, C++, SQL, HTML, and CSS. He also uses Node.js and React/Next.js heavily for backend and full stack work.",
  "What are his strengths?":
  ...
  "How can I contact him?":
    "You can reach Vedanand at pvedanad2003@gmail.com. Social links for GitHub, LinkedIn, and LeetCode are available in the contact section.",
};
...
function getAssistantReply(prompt: string) {
  ...
  if (!text) {
    return {
      text: "Please type a question about Vedanand's profile, projects, skills, or contact details.",
    };
  }

  const exact = quickAnswers[text];
  if (exact) {
    if (text === "Who is Vedanand?") {
      return {
        text: `${exact} Would you like to know more about his background, projects, or experience?`,
        suggestion: {
          title: "About Vedanand",
          description: "Learn more about Vedanand's background, profile, and technical strengths.",
          href: "/about",
        },
      };
    }
  ...
  if (normalized.includes("project")) {
    return {
      text: "Highlighted projects include Smart Cloud Storage System, airbnb complete backend (clone) and a real-time collaborative drawing app.",
      suggestion: {
  ...
  if (normalized.includes("skill") || normalized.includes("stack") || normalized.includes("tech")) {
    return {
      text: "Vedanand's stack includes Node.js, Express.js, Kafka, WebSockets, PostgreSQL, Redis, Docker, React, Next.js, and TypeScript.",
      suggestion: {
  ...
  if (normalized.includes("education") || normalized.includes("college")) {
    return {
      text: "Vedanand is pursuing B.Tech in Computer Science at Jaypee Institute of Information Technology (2022-2026).",
      suggestion: {
  ...
  if (normalized.includes("leetcode") || normalized.includes("achievement")) {
    return {
      text: "Achievements: 400+ LeetCode and gfg problems solved, and 92th percentile in JEE Mains among 1M+ candidates.",
      suggestion: {
        title: "Achievements",
  ...
  if (normalized.includes("contact") || normalized.includes("email")) {
    return {
      text: "Contact: pvedanand2003@gmail.com. You can also use the social links in the contact section.",
      suggestion: {
        title: "Go to Contact",
  ...
  return {
    text: "I can help with Vedanand's background, projects, skills, education, achievements, and contact details. Try one of the suggested questions above.",
    suggestion: {
...
export function HomeOverview({ chatMode = false, initialQuestion }: HomeOverviewProps) {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    const baseMessages: ChatMessage[] = [
      {
        role: "assistant",
        text: "Ask me anything about Vedanand's profile, projects, skills, or contact details.",
      },
    ];
